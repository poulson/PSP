...
main( int argc, char* argv[] )
{
    ...
    std::cout << "-------------------------------------------------------\n"
              << "Converting double-precision low-rank to Quasi2dHMatrix \n"
              << "-------------------------------------------------------" 
              << std::endl;
    try
    {
    ...
    try
    {
        psp::LowRankMatrix<std::complex<double>,false> F;
        F.U.Resize( m, r );
        F.V.Resize( n, r );
        for( int j=0; j<r; ++j )
            for( int i=0; i<m; ++i )
        ...
        H.AdjointMapVector( std::complex<double>(4.0,5.0), x, y );
