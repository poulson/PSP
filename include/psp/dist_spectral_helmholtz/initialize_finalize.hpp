...
template<typename R>
void
DistSpectralHelmholtz<R>::Finalize()
{
...
template<typename R>
Complex<R>
DistSpectralHelmholtz<R>::s1Inv( int x ) const
{
    const int bx = disc_.bx;
    const R etax = bx*hx_;
...
template<typename R>
Complex<R>
DistSpectralHelmholtz<R>::s2Inv( int y ) const
{
    const int by = disc_.by;
    const R etay = by*hy_;
...
template<typename R>
Complex<R>
DistSpectralHelmholtz<R>::s3Inv( int v ) const
{
    const int bz = disc_.bz;
    const R etaz = bz*hz_;
    if( v < bz-1 )
...
template<typename R>
Complex<R>
DistSpectralHelmholtz<R>::s3InvArtificial( int v, int vOffset ) const
{
    const int bz = disc_.bz;
    const R etaz = bz*hz_;
    if( v < vOffset+bz-1 )
...
// TODO: Switch to Paul's code
template<typename R>
void
DistSpectralHelmholtz<R>::FormGlobalRow
...
template<typename R>
void
DistSpectralHelmholtz<R>::FormLowerColumnOfNode
...
template<typename R>        
void
DistSpectralHelmholtz<R>::FillPanelFronts
( int vOffset, int vSize, 
  const cliq::DistSymmInfo& info,
