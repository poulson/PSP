                ...
                DistQuasi2dHMat<Scalar,Conjugated>& C )
{
    ...
    case SPLIT_LOW_RANK:
    {
        ...
        case DENSE:
        {
            ...
            const int n = B.Width();
            C._VMap[key] = new Dense<Scalar>( n, SFA.rank );
    ...
    case LOW_RANK:
    {
        ...
        case DENSE:
        {
            ...
            const int k = B.Height();
            const int n = B.Width();
            C._VMap[key] = new Dense<Scalar>( n, FA.Rank() );
            const Dense<Scalar>& DB = *B._block.data.D;
            Dense<Scalar>& VC = *C._VMap[key];
            const char option = ( Conjugated ? 'C' : 'T' );
            blas::Gemm
    ...
    case SPLIT_DENSE:
    {
        ...
        case SPLIT_LOW_RANK:
        {
            ...
            if( A._inSourceTeam )
            {
                ...
                if( m != 0 && SFB.rank != 0 )
                {
                    C._ZMap[key] = new Dense<Scalar>( m, SFB.rank );
                    Dense<Scalar>& ZC = *C._ZMap[key];
                    blas::Gemm
                    ( 'N', 'N', m, SFB.rank, k,
        ...
        case LOW_RANK:
        {
            ...
            if( m != 0 && FB.Rank() != 0 )
            {
                C._ZMap[key] = new Dense<Scalar>( m, FB.Rank() );
                Dense<Scalar>& ZC = *C._ZMap[key];
                blas::Gemm
                ( 'N', 'N', m, FB.Rank(), k,
                  alpha,     SDA.D.LockedBuffer(), SDA.D.LDim(),
    ...
    case DENSE:
    {
        ...
        case SPLIT_LOW_RANK:
        {
            ...
            const int m = A.Height();
            const int k = A.Width();
            C._UMap[key] = new Dense<Scalar>( m, SFB.rank );
                      ...
                      DistQuasi2dHMat<Scalar,Conjugated>& C ) const
{
    ...
    case LOW_RANK:
    {
        ...
        case LOW_RANK:
        {
            ...
            const int m = A.Height();
            const int k = A.Width();
            C._UMap[key] = new Dense<Scalar>( m, FB.Rank() );
            C._VMap[key] = new Dense<Scalar>;
            Dense<Scalar>& UC = *C._UMap[key];
            Dense<Scalar>& VC = *C._VMap[key];
            if( FB.Rank() != 0 && k != 0 )
    ...
    case SPLIT_DENSE:
    {
        ...
        case SPLIT_LOW_RANK:
            ...
            if( A._inTargetTeam )
            {
                ...
                if( A.Height() != 0 && SFB.rank != 0 )
                    hmat_tools::Copy( *C._ZMap[key], UC );
                else
