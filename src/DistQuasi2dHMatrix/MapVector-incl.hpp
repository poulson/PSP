                      ...
                      Vector<Scalar>& yLocal ) const
{
    ...
    case DIST_NODE:
    {
        ...
        if( teamSize == 2 )
        {
            if( teamRank == 0 )     
            {
                ...
                for( int t=0; t<2; ++t )
                    for( int s=0; s<2; ++s )
                        node.Child(t,s).MapVectorPassData
                        ( nodeContext.Child(t,s), alpha, xLocal, yLocal );
            }
            else
            {
        ...
        else // teamSize >= 4
        {
            ...
            case 3:
                // Take care of the work specific to our subteams
                node.Child(3,3).MapVectorPassData
    ...
    case SPLIT_NODE:
    {
        ...
        else
        {
            ...
            for( int t=0; t<4; ++t )
                for( int s=0; s<4; ++s )
                    node.Child(t,s).MapVectorPassDataSplitNodeUnpack
                    ( head, nodeContext.Child(t,s) );
        }
        break;
    }
...
template<typename Scalar,bool Conjugated>
void
psp::DistQuasi2dHMatrix<Scalar,Conjugated>::MapVectorPassDataSplitNodeCount
( std::size_t& bufferSize ) const
{
#ifndef RELEASE
    PushCallStack("DistQuasi2dHMatrix::MapVectorPassDataSplitNodeCount");
#endif
    switch( _block.type )
    {
    case SPLIT_NODE:
...
template<typename Scalar,bool Conjugated>
void
psp::DistQuasi2dHMatrix<Scalar,Conjugated>::MapVectorPassDataSplitNodePack
( byte*& head, const MapVectorContext& context ) const
{
#ifndef RELEASE
    PushCallStack("DistQuasi2dHMatrix::MapVectorPassDataSplitNodePack");
    if( !_inSourceTeam )
    ...
    {
        const Node& node = *_block.data.N;
        const typename MapVectorContext::DistNodeContext& nodeContext = 
            *context.block.data.DN;
        for( int t=0; t<4; ++t )
            for( int s=0; s<4; ++s )
                node.Child(t,s).MapVectorPassDataSplitNodePack
...
template<typename Scalar,bool Conjugated>
void
psp::DistQuasi2dHMatrix<Scalar,Conjugated>::MapVectorPassDataSplitNodeUnpack
( const byte*& head, MapVectorContext& context ) const
{
#ifndef RELEASE
    PushCallStack("DistQuasi2dHMatrix::MapVectorPassDataSplitNodeUnpack");
    if( !_inTargetTeam )
                      ...
                      Vector<Scalar>& yLocal ) const
{
    ...
    case DIST_NODE:
    {
        ...
        if( teamSize == 2 )
        {
            if( teamRank == 0 )     
            {
                ...
                for( int t=0; t<2; ++t )
                    for( int s=0; s<2; ++s )
                        node.Child(t,s).TransposeMapVectorPassData
                        ( nodeContext.Child(t,s), alpha, xLocal, yLocal );
            }
            else
            {
        ...
        else // teamSize >= 4
        {
            ...
            case 0:
                // Take care of the work specific to our subteams
                node.Child(0,0).TransposeMapVectorPassData
                ( nodeContext.Child(0,0), alpha, xLocal, yLocal );
                // Interact with subteam 1
                node.Child(0,1).TransposeMapVectorPassData
                ( nodeContext.Child(0,1), alpha, xLocal, yLocal );
                node.Child(1,0).TransposeMapVectorPassData
                ( nodeContext.Child(1,0), alpha, xLocal, yLocal );
                // Interact with subteam 2
                node.Child(0,2).TransposeMapVectorPassData
                ( nodeContext.Child(0,2), alpha, xLocal, yLocal );
                node.Child(2,0).TransposeMapVectorPassData
                ( nodeContext.Child(2,0), alpha, xLocal, yLocal );
                // Interact with subteam 3
                node.Child(0,3).TransposeMapVectorPassData
                ( nodeContext.Child(0,3), alpha, xLocal, yLocal );
                node.Child(3,0).TransposeMapVectorPassData
                ( nodeContext.Child(3,0), alpha, xLocal, yLocal );
                break;
            case 1:
                // Take care of the work specific to our subteams
                node.Child(1,1).TransposeMapVectorPassData
                ( nodeContext.Child(1,1), alpha, xLocal, yLocal );
                // Interact with subteam 0
                node.Child(0,1).TransposeMapVectorPassData
                ( nodeContext.Child(0,1), alpha, xLocal, yLocal );
                ...
                ( nodeContext.Child(1,0), alpha, xLocal, yLocal );
                // Interact with subteam 3
                node.Child(1,3).TransposeMapVectorPassData
                ( nodeContext.Child(1,3), alpha, xLocal, yLocal ); 
                node.Child(3,1).TransposeMapVectorPassData
                ( nodeContext.Child(3,1), alpha, xLocal, yLocal );
                // Interact with subteam 2
                node.Child(1,2).TransposeMapVectorPassData
                ( nodeContext.Child(1,2), alpha, xLocal, yLocal );
                node.Child(2,1).TransposeMapVectorPassData
                ( nodeContext.Child(2,1), alpha, xLocal, yLocal );
                break;
            case 2:
                // Take care of the work specific to our subteams
                node.Child(2,2).TransposeMapVectorPassData
                ( nodeContext.Child(2,2), alpha, xLocal, yLocal );
                // Interact with subteam 3
                node.Child(2,3).TransposeMapVectorPassData
                ( nodeContext.Child(2,3), alpha, xLocal, yLocal );
                node.Child(3,2).TransposeMapVectorPassData
                ( nodeContext.Child(3,2), alpha, xLocal, yLocal );
                // Interact with subteam 0
                node.Child(0,2).TransposeMapVectorPassData
                ( nodeContext.Child(0,2), alpha, xLocal, yLocal );
            ...
            case 3:
                // Take care of the work specific to our subteams
                node.Child(3,3).TransposeMapVectorPassData
                ( nodeContext.Child(3,3), alpha, xLocal, yLocal );
                // Interact with subteam 2
                node.Child(2,3).TransposeMapVectorPassData
                ( nodeContext.Child(2,3), alpha, xLocal, yLocal );
    ...
    case SPLIT_NODE:
    {
        ...
        else
        {
            ...
            for( int t=0; t<4; ++t )
                for( int s=0; s<4; ++s )
                    node.Child(t,s).TransposeMapVectorPassDataSplitNodeUnpack
                    ( head, nodeContext.Child(t,s) );
        }
        break;
    }
...
void
psp::DistQuasi2dHMatrix<Scalar,Conjugated>::
TransposeMapVectorPassDataSplitNodeCount
( std::size_t& bufferSize ) const
{
#ifndef RELEASE
    PushCallStack
    ("DistQuasi2dHMatrix::TransposeMapVectorPassDataSplitNodeCount");
#endif
    switch( _block.type )
    {
    case SPLIT_NODE:
...
void
psp::DistQuasi2dHMatrix<Scalar,Conjugated>::
TransposeMapVectorPassDataSplitNodePack
( byte*& head, const MapVectorContext& context,
  const Vector<Scalar>& xLocal ) const
{
#ifndef RELEASE
    PushCallStack
    ("DistQuasi2dHMatrix::TransposeMapVectorPassDataSplitNodePack");
    if( !_inTargetTeam )
        throw std::logic_error("Calling process should be in target team");
#endif
    switch( _block.type )
...
void
psp::DistQuasi2dHMatrix<Scalar,Conjugated>::
TransposeMapVectorPassDataSplitNodeUnpack
( const byte*& head, MapVectorContext& context ) const
{
#ifndef RELEASE
    PushCallStack
    ("DistQuasi2dHMatrix::TransposeMapVectorPassDataSplitNodeUnpack");
    if( !_inSourceTeam )
        throw std::logic_error("Calling process should be in source team");
#endif
    switch( _block.type )
